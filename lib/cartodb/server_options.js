...
/* This whole block is about generating X-Cache-Channel { */
    ...
    me.findLastUpdated = function (username, api_key, tableNames, callback) {
      var md5Names = tableNames.map(function(v) { return me.generateMD5(v); });
      var sql = 'SELECT EXTRACT(EPOCH FROM max(updated_at)) as max FROM CDB_TableMetadataView m WHERE m.md5tabname = any (\'{'
        + md5Names.join(',') + '}\')';
